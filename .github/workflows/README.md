...
**Railway Free 플랜의 제한사항**:
- 15분 동안 HTTP 요청이 없으면 서버가 자동으로 슬립(sleep) 모드로 전환됩니다
- 슬립 상태에서 다음 요청이 오면 서버가 깨어나는데 약 30초~1분 소요됩니다
- 이로 인해 정해진 시간에 실행되어야 하는 cron job이 지연되거나 실패할 수 있습니다

**해결 방법**:
- 중요한 배치 작업(cron job) 실행 1분 전에 GitHub Actions로 서버를 미리 깨워둡니다
- 서버가 이미 깨어있는 상태에서 cron job이 정확한 시간에 실행되도록 보장합니다

## 워크플로우 목록

### 1. Daily AI Analysis (`daily-ai-analysis.yml`)
- **실행 시간**: 매일 새벽 2시 59분 (KST) - **1회 실행**
- **목적**: 3시에 실행되는 AI 분석 cron job을 위해 서버를 미리 깨움
- **동작**: `/health` 엔드포인트를 호출하여 서버를 활성화
- **결과**: 3시 정각에 AI 분석이 지연 없이 실행됨

### 2. Daily Push Notification (`daily-push-notification.yml`)
- **실행 시간**: 매일 아침 8시 29분 (KST) - **1회 실행**
- **목적**: 8시 30분에 실행되는 푸시 알림 cron job을 위해 서버를 미리 깨움
- **동작**: `/health` 엔드포인트를 호출하여 서버를 활성화
- **결과**: 8시 30분에 사용자들에게 푸시 알림이 정시에 발송됨

### 3. Daily Backup (`daily-backup.yml`)
- **실행 시간**: 매일 새벽 3시 59분 (KST) - **1회 실행**
- **목적**: 4시에 실행되는 데이터베이스 백업 cron job을 위해 서버를 미리 깨움
- **동작**: `/health` 엔드포인트를 호출하여 서버를 활성화
- **결과**: 4시 정각에 데이터베이스 백업이 지연 없이 실행됨
