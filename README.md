...
- 🌟 **도장 시스템**: AI가 일기를 분석하여 선생님 도장을 찍어줍니다
- 📊 **통계 및 리포트**: 주간 감정 분석 리포트를 확인할 수 있습니다 (월간 리포트는 추후 추가 예정)
...
2. **AI 코멘트 받기**
   ...
   - 매일 새벽 3시에 배치 작업이 실행되어 AI 선생님이 코멘트를 달아줍니다
   - 다음날 아침 8시 30분에 푸시 알림을 받고, 앱을 열면 AI 코멘트와 도장을 확인할 수 있습니다
...
서버에서 자동으로 실행되는 배치 작업 (node-cron 기반):

| 시간 | 작업 | 설명 |
|------|------|------|
| **새벽 3시** | AI 분석 | AI 코멘트가 없는 일기를 Claude API로 분석하여 스탬프 추천 및 코멘트 생성 |
| **새벽 4시** | 자동 백업 | 데이터베이스 + 이미지 파일 백업 (로컬 + S3 이중 저장) |
| **아침 8시 30분** | 푸시 알림 | 전날 AI 코멘트를 받은 사용자에게 알림 전송 (Firebase Cloud Messaging) |

**참고**: Railway Free 플랜은 15분간 요청이 없으면 서버가 슬립 모드로 전환되므로, GitHub Actions가 배치 작업 1분 전에 서버를 깨웁니다. (자세한 내용: [`.github/workflows/README.md`](.github/workflows/README.md))
...
Railway Free 플랜은 15분간 요청이 없으면 슬립 모드로 전환됩니다.
...
- [x] **푸시 알림** - Firebase Cloud Messaging
  - AI 코멘트 완료 시 자동 알림 (매일 8시 30분)
  - 개별/전체 사용자 알림 지원
  - 전송 결과 확인 및 실패한 토큰 자동 삭제
- [x] **AWS S3 통합** ⭐
  - 일기 이미지 S3 저장
  - 백업 파일 S3 저장
